...
class Signal_Generator:
    ...
    def printing_parameters(self):
        ...
        if self.A is not None and self.omega is not None and self.theta is not None:
            ...
            theta_norm = normalize(self.theta, 0, 2 * np.pi)  # Phase range: 0 to 2π
            
            # Combine normalized parameters into sources
            sources = list(zip(A_norm, omega_norm, theta_norm))
            ...
            params = []
            for amp_norm, omega_norm, theta_norm in sorted_sources:
                params.extend([amp_norm, omega_norm, theta_norm])
        else:
            params = []
            print("Some parameters have not been generated yet.")
